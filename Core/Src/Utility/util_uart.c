...
void Uart_RxIdleCallback(UART_HandleTypeDef* huart) {
   ...
   if (huart == Const_Communicate_UART_HANDLER) 
      Communicate_RXCallback(huart);

	  if (huart == Const_Motor_UART_HANDLER)
		Motor_EncoderDecodeCallback(NULL,huart ,0,NULL,0);
		
		if (huart == Const_DT35_UART_HANDLER)
		DT35_RXCallback(huart);
